...
let rec lex_ident_body acc = let b = lexer_getc () in if is_ident_cont b then lex_ident_body (b :: acc) else (acc, b)
...
let lex_utf8_e2 acc = let b2 = lexer_getc () in let b3 = lexer_getc () in if b2 = 150 && b3 = 170 then (TMint :: acc, 0) else if b2 = 134 && b3 = 144 then (TLArrow :: acc, 0) else if b2 = 134 && b3 = 146 then (TRArrow :: acc, 0) else if b2 = 159 && b3 = 181 then (TLArrow :: acc, 0) else if b2 = 159 && b3 = 182 then (TRArrow :: acc, 0) else (TUnknown 226 :: acc, 0)
let lex_other b acc = if is_letter b then lex_ident_after [b] acc else if is_digit b then let pair = lex_digits (b - 48) in let n = fst pair in let next = snd pair in if next = 37 then (TPct n :: acc, 0) else (TInt n :: acc, next) else if b = 40 then (TLParen :: acc, 0) else if b = 41 then (TRParen :: acc, 0) else if b = 123 then (TLBrace :: acc, 0) else if b = 125 then (TRBrace :: acc, 0) else if b = 44 then (TComma :: acc, 0) else if b = 42 then (TMint :: acc, 0) else if b = 170 then (TMint :: acc, 0) else if b = 144 then (TLArrow :: acc, 0) else if b = 245 then (TLArrow :: acc, 0) else if b = 146 then (TRArrow :: acc, 0) else if b = 210 then (TUnknown 62 :: TRArrow :: acc, 0) else if b = 226 then lex_utf8_e2 acc else (TUnknown b :: acc, 0)
