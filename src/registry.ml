...
let rec match_template_items template toks slots consumed = match template with [] -> SyntaxMatch ("", [], [], List.rev slots, consumed) | th :: tt -> (match toks with [] -> NoSyntaxMatch | ih :: it -> if is_slot th then match_template_items tt it (ih :: slots) (consumed + 1) else if th = ih then match_template_items tt it slots (consumed + 1) else NoSyntaxMatch)
...
let match_len m = match m with NoSyntaxMatch -> 0 | SyntaxMatch (mode, template, expansion, slots, consumed) -> consumed
