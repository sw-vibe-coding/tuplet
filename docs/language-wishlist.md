...
extensible via the `▪syntax T expand E` mechanism (ASCII fallback
`*syntax T expand E`; the kernel's single load-bearing primitive);
nothing requires changing the
compiler or runtime.
...
| 2     | core-arith        | `+ - * /`, `<`, `<=`, `>=`, `=`, `<>`, `max`, `min`, `div`, `max₂`, `min₂`, `div₂` | 0, 1 |
...
naming convention as the OCaml host. Top-level ASCII fallback mints
or canonical top-level `▪name` declarations in `math.tup` become
`Math.name`.
